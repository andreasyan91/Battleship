...
int check_cpu_ships(std::string status, bool& hit, int ships_count)
{
  if(status == "Hit") {
    hit = true;
  }
...
void cpu_turn(int& ships_count)
{
  ...
  while(hit) {
    ...
    ships_count = check_cpu_ships(status, hit, ships_count);
    if(ships_count == 0) {
     hit = false; 
     }
  }
...
void shoot(char** sea, int** ships)
{
  ...
  while(continue_game) {
    ...
    std::cout<<"shoot"<<cpu_ships_count<<std::endl;
...
/// Calls set_direction, set_on_board, print_real_sea and shoot functions to
//start the game.
void battleship()
{
...
/// Receives a command from the player to start close the game.
void start()
{
